...
3. event 一覧から対象 event を選択する
  - event を選び直した時点で、event alias 入力欄はクリアされます（前の入力値は引き継がれません）
4. 必要に応じて event alias を入力してローカルスナップショットとして保存する
5. 「大会一覧」から該当 event を読み込む
6. 「大会管理」で alias、1P/2P ルール、カテゴリ設定を整える
7. 「ブラケット」で試合の結果を入力してローカル保存する
8. 確定済み set をまとめて start.gg に反映する
9. 必要に応じて OBS overlay、メッセージ、呼び出しリスト、プレイヤーカードを使う
